...
class MarxanAnalysis(object):
    ...
    def write_bound(self):
        if not settings.USE_BLM:
            return

        out = "%s/data/bound.dat" % self.outdir
        template = os.path.join(self.templatedir, 'bound.dat')

        if not os.path.exists(template):
            raise Exception("bound.dat template required!")

        if settings.VARIABLE_GEOGRAPHY:
            raise Exception("bound.dat needs to be tested for variable geographies")
        else:
            # Assume the template is 100% ready to go
            copyfile(template, out)
...
CLUMPTYPE 0
VERBOSITY 3
""" % (blm, self.NUMREPS, self.NUMITNS, bounds, self.name)
